...
ShockwaveTextureGpuProgramWrapper::ShockwaveTextureGpuProgramWrapper() : m_fTimeElapsed(0)
{
    m_center = std::unique_ptr<Vector2D>(new Vector2D());
}

void ShockwaveTextureGpuProgramWrapper::configure(float x, float y, float timeElapsed)
{
    m_center->set(x, y);
