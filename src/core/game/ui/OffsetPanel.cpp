...
OffsetPanel::OffsetPanel(float x, float y, float width, float height) : PhysicalEntity(x, y, width, height), m_iOffset(0), m_isOpen(false)
{
    ...
    m_leftTextPosition = std::unique_ptr<Vector2D>(new Vector2D(m_offsetLeftButton->getLowerLeft().getX() + m_offsetLeftButton->getWidth() - m_offsetLeftButton->getWidth() / 3, m_offsetLeftButton->getLowerLeft().getY() + m_offsetLeftButton->getHeight() / 2));
    m_rightTextPosition = std::unique_ptr<Vector2D>(new Vector2D(m_offsetRightButton->getLowerLeft().getX() + m_offsetRightButton->getWidth() - m_offsetRightButton->getWidth() / 3, m_offsetRightButton->getLowerLeft().getY() + m_offsetRightButton->getHeight() / 2));
}
